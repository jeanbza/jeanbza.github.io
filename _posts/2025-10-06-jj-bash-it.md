...
description: "A bash-it plugin that surfaces jj repository state in your shell prompt."
toc: true
---
...
1. Create `~/.bash_it/custom/jj.plugins.bash` with:
    ...
    if [ "$SCM_PROMPT_SHOW_JJ_PRIVATE_INFO" = "true" ] && is_jj_repo; then
        ...
        # Fallback: if the command fails, default to the first character
        if [[ -z "$prefix" ]]; then
        ...
        echo -e "${SCM_THEME_PROMPT_PREFIX}${jj_info}${SCM_THEME_PROMPT_SUFFIX}"
    fi
    }
    ```

2. Update `~/.bash_it/themes/<theme>/<theme>.bash` (ex
`~/.bash_it/themes/sexy/sexy.bash`), amending the prompt_command:

    ```sh
    function prompt_command() {
    local scm_info=""

    # 1. Check if it is a JJ repo first (Priority)
    # We use the 'is_jj_repo' function defined in your plugin.
    if type is_jj_repo &>/dev/null && is_jj_repo; then
        # If JJ, show ONLY JJ info (suppress Git)
        scm_info="\[$CYAN\]\$(jj_prompt_info)"
        
    # 2. If not JJ, check if it is a Git repo
    elif [[ -n $(git branch 2> /dev/null) ]]; then
        # If Git, show " on " + Git info
        scm_info=" on \[$PURPLE\]\$(parse_git_branch)"
    fi

    # Build the PS1 using the selected scm_info
    PS1="\[${BOLD}${MAGENTA}\]\u \[$WHITE\]at \[$ORANGE\]\h \[$WHITE\]in \[$GREEN\]\w\[$WHITE\]${scm_info}\[$WHITE\]\n\$ \[$RESET\]"
...
<br>I'm also partial to the following `jj config edit --user`:

```toml
[revsets]
log = "present(@) | ancestors(mine() & immutable_heads().., 2) | trunk()"
```

That does a good job showing me _my_ commits when I `jj log`.
